...
ssize_t palt_read(struct file *file, char __user *buf, size_t count, loff_t *offp)
{
	printk(KERN_INFO "plat read data\n");
	return 0;
}

ssize_t plat_write(struct file *file, const char __user *buf, size_t count, loff_t *offp)
{
	printk(KERN_INFO " write data is %s\n", buf);
	
	return 0;
}

static int plat_release(struct inode *inode, struct file *file)
{
	printk(KERN_INFO "plat release\n");
	return 0;
}

static int plat_open(struct inode *inode, struct file *file)
{
	printk(KERN_INFO "plat open\n");
	return 0;
}

static struct file_operations plat_ops = {
	.owner 		= THIS_MODULE,
	.open 		= plat_open,
...
static  struct miscdevice plat_mdev = {
